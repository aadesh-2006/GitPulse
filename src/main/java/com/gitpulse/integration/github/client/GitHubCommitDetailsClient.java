...
@Component
public class GitHubCommitDetailsClient {

    private static final Logger log = LoggerFactory.getLogger(GitHubCommitDetailsClient.class);

    private final RestClient restClient;

    public GitHubCommitDetailsClient(RestClient gitHubRestClient) {
        this.restClient = gitHubRestClient;
    }

    public GitHubCommitDetailResponse getCommitDetails(String owner, String repo, String commitSha) {
        log.debug("Fetching commit details for {}/{} commit {}", owner, repo, commitSha);
        ...
        try {
            return restClient.get()
                    ...
                    .onStatus(status -> status.value() == 404, (request, response) -> {
                        log.warn("GitHub commit not found: {}/{} [{}]", owner, repo, commitSha);
                        throw new GitHubResourceNotFoundException(owner, repo + "/commits/" + commitSha);
                    })
            ...
            log.error("Network or timeout error communicating with GitHub API for {}/{} commit {}: {}", owner, repo, commitSha, ex.getMessage());
